...
6. Symlink Drive folders into the repo
...
Links the persistent Drive directories into the repo so experiment scripts
can use relative paths like `steering_vectors/` and `results/`.
"""
...
for local, target in symlinks.items():
    ...
    if local_path.exists() or local_path.is_symlink():
        ...
        os.symlink(target, str(local_path))
...
if pathlib.Path(PAIRS_PATH).exists():
    with open(PAIRS_PATH) as f:
        pairs = json.load(f)
    print(f"Pairs file already exists — {len(pairs)} pairs loaded from {PAIRS_PATH}")
else:
    ...
    with open(PAIRS_PATH) as f:
        pairs = json.load(f)
    print(f"Built {len(pairs)} contrastive pairs → {PAIRS_PATH}")
"""
...
if pathlib.Path(VECTOR_PATH).exists():
    ...
    print(f"Steering vector already exists — optimal layer: {data.get('optimal_layer', 'N/A')}")
else:
    ...
    data = torch.load(VECTOR_PATH, weights_only=True)
    print(f"Vector computed → {VECTOR_PATH}  (optimal layer: {data.get('optimal_layer', 'N/A')})")
"""
...
PLOT_PATH = 'results/exp1_1/entropy_vs_alpha.png'
